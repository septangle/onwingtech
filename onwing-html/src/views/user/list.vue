    ...
    <Row>
      ...
      <div>
        <div style="position:relative;">
          <!-- 表格开始 -->
          <Table stripe border height="auto" :columns="columns_title" :data="user_data" ref="table"></Table>
          <!-- 表格结束 -->
          <!-- 表格内容加载提示蒙板开始 -->
...
export default {
    ...
    methods: {
        init(){
            ...
            if(access === '0'){
                ...
                this.searchCommunityID = sessionStorage.getItem('communityID');
            }
        },
        getUserDate(currentPage,pageSize) {
            ...
            axios.get(GlobalServer.findAllUser + '?page=' + currentPage + '&pageSize=' + pageSize + '&communityId=' + _this.searchCommunityID)
                .then(function(response){
                    ...
                    if(data.error === null){
                        ...
                        _this.user_data = data.userRoleCommunityDto;
        ...
        changePage(index){
            ...
            this.getUserDate(index,this.pagesize);
        },
        exportData (type) {
            if (type === 1) {
    ...
    created() {
        this.init();
        this.getUserDate(this.pageindex,this.pagesize);
    },
    beforeMount() {},
    mounted() {}
  }
